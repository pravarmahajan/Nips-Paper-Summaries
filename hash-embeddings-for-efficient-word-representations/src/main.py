...
def main():
    ...
    if parsed_args.embedding_type == "hash":
        ...
        embedding_model = StandardEmbedding(parsed_args.vocab_size, parsed_args.embedding, use_cuda)

    embedding_model = embedding_model.cuda() if use_cuda else embedding_model
    embedding_model.initializeWeights()

    model = ClassifierModel(embedding_model, num_classes, parsed_args.hidden, use_cuda)
